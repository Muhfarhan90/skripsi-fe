import { notFound } from "next/navigation";
import { CourseQuizEditorPage } from "@/features/admin/components/course-quiz-editor-page";

interface AdminCourseQuizCreatePageProps {
  params: Promise<{ id: string; sectionId: string }>;
  searchParams: Promise<{ returnTo?: string }>;
}

export default async function AdminCourseQuizCreatePage({
  params,
  searchParams,
}: AdminCourseQuizCreatePageProps) {
  const [{ id, sectionId }, query] = await Promise.all([params, searchParams]);
  const courseId = Number(id);
  const parsedSectionId = Number(sectionId);

  if (!Number.isInteger(courseId) || courseId <= 0 || !Number.isInteger(parsedSectionId) || parsedSectionId <= 0) {
    notFound();
  }

  return (
    <CourseQuizEditorPage
      courseId={courseId}
      sectionId={parsedSectionId}
      returnTo={query.returnTo}
    />
  );
}

