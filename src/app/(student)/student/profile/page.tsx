...
function StatCard({
  ...
  return (
    <article className={`flex flex-col items-center gap-1.5 rounded-2xl p-4 ${colorClass}`}>
      <Icon className="size-5 opacity-80" />
      <p className="text-2xl font-bold leading-none">{value}</p>
      <p className="text-center text-[11px] font-medium leading-tight opacity-75">{label}</p>
    </article>
  );
}
...
export default function StudentProfilePage() {
  ...
  const certificates = enrollments.filter((e) => e.has_certificate).length;
  ...
  return (
    <section className="space-y-4">
      ...
      <article className="overflow-hidden rounded-2xl bg-[var(--primary)] text-white shadow-md">
        ...
        {/* Stats row */}
        <div className="grid grid-cols-3 gap-2 border-t border-white/15 px-5 py-4">
          <StatCard icon={BookOpen} value={activeEnrollments} label="Kelas Aktif" colorClass="bg-white/15 text-white" />
          <StatCard icon={GraduationCap} value={completedEnrollments} label="Selesai" colorClass="bg-white/15 text-white" />
          <StatCard icon={Trophy} value={certificates} label="Sertifikat" colorClass="bg-white/15 text-white" />
        </div>
      </article>
      ...
      {!isEditing ? (
        <article className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-sm">
          ...
          <div className="divide-y divide-[var(--border)] px-4">
            ...
            {user?.nisn ? <InfoRow icon={User} label="NISN" value={user.nisn} /> : null}
          </div>
        </article>
      ) : null}
