...
interface AdminCourseActivityForumDetailPageProps {
  courseId: number;
  postId: number;
}
...
export function AdminCourseActivityForumDetailPage({
  courseId,
  postId,
}: AdminCourseActivityForumDetailPageProps) {
  ...
  const courseHref = buildAdminCourseActivityForumListHref(courseId);

  const courseQuery = useQuery({
    queryKey: ["admin", "course-activity", "forum", "course", courseId],
    queryFn: () => getAdminCourseById(courseId),
    enabled: Number.isFinite(courseId) && courseId > 0,
  });

  const selectedPostQuery = useQuery({
    queryKey: ["admin", "course", courseId, "forum-post", postId],
    queryFn: () => getAdminCourseForumPost(courseId, postId),
    enabled: Number.isFinite(courseId) && courseId > 0 && Number.isFinite(postId) && postId > 0,
  });

  const createReplyMutation = useMutation({
    mutationFn: (payload: { content: string }) => createAdminCourseForumReply(courseId, postId, payload),
    onSuccess: () => {
      setReplyContent("");
  ...
  const updatePostMutation = useMutation({
    mutationFn: (payload: { title: string; content: string }) =>
      updateAdminCourseForumPost(courseId, postId, payload),
    onSuccess: () => {
  ...
  const togglePinMutation = useMutation({
    mutationFn: () => toggleAdminCourseForumPostPin(courseId, postId),
    onSuccess: (post) => {
  ...
  const deletePostMutation = useMutation({
    mutationFn: () => deleteAdminCourseForumPost(courseId, postId),
    onSuccess: () => {
  ...
  const courseTitle = courseQuery.data?.title ?? `Course #${courseId}`;
  ...
  return (
    <section className="space-y-5">
      <AdminPageHeader
        title="Detail Forum"
        description={`Moderasi diskusi dan balasan untuk course ${courseTitle}.`}
      />
      ...
      <Card className="border border-[var(--border)] bg-[var(--card)] shadow-sm">
        <CardHeader className="gap-3 pb-2">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <Button render={<Link href={courseHref} />} type="button" variant="outline">
      ...
      <Card className="border border-[var(--border)] bg-[var(--card)] shadow-sm">
        ...
        <CardContent className="space-y-4">
          {selectedPostQuery.isLoading ? (
            ...
            <div className="rounded-xl border border-dashed border-[var(--border)] bg-[var(--surface-soft)] p-6 text-sm text-[var(--danger-soft-foreground)]">
              Detail topik forum belum bisa dimuat.
            </div>
          ) : (
                  ...
                  <div>
                    ...
                    {isEditingSelectedPost ? (
                      <Input
                        ...
                        className="mt-3 w-full min-w-[320px] border-slate-300 bg-white shadow-sm focus-visible:border-slate-400 sm:min-w-[460px] lg:min-w-[560px]"
                      />
                    ) : (
